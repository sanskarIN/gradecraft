...
`tests/gradeMath.test.ts`, `tests/gradeProperties.test.ts`, `tests/gpa.test.ts`, `tests/validation.test.ts`, and `tests/whatIf.test.ts` cover calculation, deterministic property-style invariants, target planning, bounded inputs, calendar-date validation, category-name uniqueness, scale validation, and edge behavior.

## Data integration tests

CSV, backups, schema restoration, migrations, structured-log redaction, and Local Storage behavior are tested in jsdom. CSV regression cases cover spreadsheet-formula hardening, secure text round trips, bounded names, valid calendar dates, and duplicate-header rejection. Storage tests cover corrupt primary data, recovery-copy repair, invalid-primary backup protection, blocked storage access, and explicit-delete semantics.

`tests/securityMetadata.test.ts` verifies the static browser CSP/referrer baseline. `tests/serviceWorkerSource.test.ts` guards same-origin/scope restrictions plus ownership isolation for cache cleanup and cache reads.

## Component tests

`tests/App.test.tsx` verifies first-run onboarding isolation/focus, navigation landmarks, and persistence-failure warnings. `tests/AppContext.test.tsx` verifies explicit local-data deletion stays cleared during the reset operation. `tests/CourseForm.test.tsx` protects assignment/category referential integrity. `tests/CoursePage.test.tsx` protects assignment undo from recreating a reference to a category removed after deletion. `tests/Modal.test.tsx` covers draft reset, controlled close behavior, and accessible dialog naming. `tests/WhatIfPage.test.tsx` covers stale course-route recovery. `tests/DataPage.test.tsx` covers CSV course selection after data replacement.
