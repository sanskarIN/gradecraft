...
- Tagged-release tag/version matching and SHA-256 checksum publication for the PWA archive.
- Static release-workflow regression coverage protecting checksum generation and native GitHub CLI publishing.
...
- Service-worker cache cleanup and reads are isolated to GradeCraft-owned caches so unrelated same-origin application caches are not removed or read.
...
- Settings includes direct privacy/data-transfer and About entry points in addition to appearance/accessibility/update controls.
...
- Tagged releases use GitHub's authenticated `gh` CLI instead of a third-party release-publishing action.
...
- CSV import rejects duplicate column names, assignment counts above the bounded import limit, explicit category weights that conflict with an existing course category, quotes beginning in the middle of unquoted fields, and non-whitespace text after closing quotes.
