...
const requiredScripts=["build","typecheck","lint","format:check","docs:links","security:secrets","test","test:e2e","verify","release:gate"];
const failures=[];
...
for(const command of ["npm run typecheck","npm run lint","npm run format:check","npm run docs:links","npm run security:secrets","npm test","npm run build","npm audit --audit-level=high"])if(!ci.includes(command))failures.push(`CI is missing quality gate: ${command}`);
